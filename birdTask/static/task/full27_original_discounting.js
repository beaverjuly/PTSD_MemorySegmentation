 ...
 * timeline of 30 discounting trials (plus a one‑time instruction page
...
(function() {
  ...
  function ensureDiscountingFrame(display_element, skyUrl, landUrl) {
    ...
    var skyStyle = '';
    if (skyUrl) {
      skyStyle = "background-image: url('" + skyUrl + "'); background-size: cover; background-position: center top; background-repeat: no-repeat;";
    }
    var landStyle = '';
    if (landUrl) {
      landStyle = "background-image: url('" + landUrl + "'); background-size: cover; background-position: center bottom; background-repeat: no-repeat;";
    }
    ...
    display_element.innerHTML =
      '<div id="dd-frame" class="game-container">' +
        ...
        '<div class="main-container" style="position:relative;">' +
          ...
          '<div id="dd-stage" style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); width:100%; display:flex; justify-content:center;"></div>' +
        '</div>' +
      '</div>';
  }
  ...
  jsPsych.plugins['delay-discounting'] = (function() {
    ...
    plugin.trial = function(display_element, trial) {
      ...
      function renderChoiceUI() {
        var html = '';
        html += '<div class="discount-container" style="display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center;">';
        html += '<p style="font-size:28px; margin-bottom:30px;">Choose the option you prefer</p>';
        html += '<div style="display:flex; flex-direction:row; justify-content:center; align-items:center; gap:200px; font-size:40px; font-weight:normal;">';
        html +=   '<div style="display:flex; flex-direction:column; align-items:center;">';
        html +=     '<div id="dd-left" style="border:2px solid #555; padding:20px 40px; border-radius:8px;">$' + Math.round(trial.SS_amount) + '<br><span style="font-size:20px;">now</span></div>';
        html +=     '<div style="font-size:24px; font-weight:bold; margin-top:10px; color:#333;">1</div>';
        html +=   '</div>';
        html +=   '<div style="display:flex; flex-direction:column; align-items:center;">';
        html +=     '<div id="dd-right" style="border:2px solid #555; padding:20px 40px; border-radius:8px;">$' + Math.round(trial.LL_amount) + '<br><span style="font-size:20px;">in ' + trial.delay_days + ' days</span></div>';
      ...
      function handleTimeout() {
        ...
        var html = '';
        html += '<div class="discount-container" style="display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; border:4px solid red; padding:10px 20px; border-radius:8px;">';
        html += '<p style="font-size:28px; margin-bottom:20px;">Choose the option you prefer</p>';
        html += '<p style="font-size:24px; margin-top:10px; color:#d9534f;">No selection made. Please make a selection within the allowed time.</p>';
        html += '</div>';
        setStageHTML(display_element, html);
  ...
  jsPsych.plugins['delay-discounting-instructions'] = (function() {
    ...
    plugin.trial = function(display_element, trial) {
      ...
      var html = '<div style="display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center;">';
      html += '<p style="font-size:28px;">' + trial.pages[0] + '</p>';
      html += '<button id="dd-instr-continue" style="margin-top:20px; padding:10px 20px; font-size:20px;">Continue</button>';
      html += '</div>';
      setStageHTML(display_element, html);
  ...
  async function runDelayDiscounting(conditionTag) {
    ...
    timeline.push({
      ...
      pages: [
        'You will now see a series of hypothetical choices. Each choice will consist of two monetary rewards, received at different points in time. <br><br>Chose the option that you would most prefer to receive by pressing 1 or 2 on your keyboard.'
      ],
      bg_sky: skyUrl,
      bg_land: landUrl
    });
