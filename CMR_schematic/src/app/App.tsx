...
{(() => {
  ...
  const iH = 40;
  const startX = px + 55;
  const positions = [0, 85, 275, 360];
  const hBeltX = px + 25;
  const hBeltW = pw - 50;

  const beltT = (x: number) => (x - hBeltX) / hBeltW;

  const itemLabels = ["item j-2", "item j-1", "item j", "item j+1"];
  ...
  return (
    <g>
      ...
      {/* Items — highlight item j as the first item after the boundary */}
      {positions.map((offset, i) => {
        const cx = startX + offset;
        const isBoundaryItem = i === 2; // item j
        const t = beltT(cx);
        const col = isBoundaryItem ? orange : sampleCividis(t);
        const bgFill = isBoundaryItem ? `${orange}20` : sampleCividis(t);
        const txtCol = isBoundaryItem ? orange : cividisTextColor(t);
        const strokeCol = isBoundaryItem ? orange : sampleCividis(Math.max(0, t - 0.05));
        ...
        return (
          <g key={i}>
            ...
            <rect
              ...
              fill={bgFill}
              stroke={strokeCol}
              strokeWidth={isBoundaryItem ? 2 : 1.2}
              opacity={isBoundaryItem ? 1 : 0.85}
            />
            <text
              x={cx}
              y={iY + iH / 2 + 5}
      ...
      {/* Spacing brackets */}
      {[
        { x1: startX + positions[0], x2: startX + positions[1], big: false },
        { x1: startX + positions[1], x2: startX + positions[2], big: true },  // j-1 -> j
        { x1: startX + positions[2], x2: startX + positions[3], big: false }, // j -> j+1
      ].map((seg, i) => {
        const bracketY = bY + bH + 20;
        ...
        return (
          <g key={i} opacity={seg.big ? 0.75 : 0.5}>
            ...
            <text
              ...
              textAnchor="middle"
              fill={col}
              fontSize={seg.big ? 12 : 11}
              fontWeight="500"
            >
              {seg.big ? "larger Δc" : "Δc"}
            </text>
          </g>
        );
      })}
      ...
      <text
        ...
        fontWeight="500"
      >
        Larger context shift before item j creates a boundary
      </text>
    </g>
  );
})()}
